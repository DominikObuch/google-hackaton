...
deploy-backend:
	...
	gcloud run deploy buildwithai-backend \
		...
		--set-env-vars=DB_HOST="/cloudsql/$(GCP_PROJECT):$(REGION):$(DB_INSTANCE)",DB_PORT=5432,DB_USERNAME=$(DB_USER),DB_PASSWORD=$(DB_PASSWORD),DB_NAME=$(DB_NAME),ADK_AGENT_URL=$(AGENT_URL)
